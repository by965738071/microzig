...
pub const Options = struct {
    ...
    clock_device: mdf.base.ClockDevice,
    motor_steps: u16 = 200,
};
...
pub fn Stepper(comptime Driver: type) type {
    return struct {
        const Self = @This();
        pub const SpeedProfile = union(enum) {
            constant_speed,
            linear_speed: struct {
                accel: u16 = 1000,
                decel: u16 = 1000,
        ...
        pub fn set_speed_profile(self: *Self, profile: SpeedProfile) void {
...
// Testing
const Test_GPIO = mdf.base.Digital_IO.TestDevice;
const TestTime = mdf.base.ClockDevice.TestDevice;

test "begin" {
    var dp = Test_GPIO.init(.output, .high);
    var sp = Test_GPIO.init(.output, .high);
...
test "A4988: set microstep with ms pins set" {
    var dp = Test_GPIO.init(.output, .high);
    var sp = Test_GPIO.init(.output, .high);
    var ms1 = Test_GPIO.init(.output, .high);
    var ms2 = Test_GPIO.init(.output, .high);
    var ms3 = Test_GPIO.init(.output, .high);
    var ttd = TestTime.init();
    var stepper = Stepper(A4988).init(.{
        .dir_pin = dp.digital_io(),
        .step_pin = sp.digital_io(),
...
test "A4988: set microstep without ms pins set" {
    ...
    try std.testing.expectError(error.Pins, rv);
    const tcs = .{
    ...
    try std.testing.expectError(error.Pins, stepper.set_microstep(1));
}
